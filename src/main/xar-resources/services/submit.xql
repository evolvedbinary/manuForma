...
declare function local:create-id() as xs:string {
    let $base-uri := 
            if (contains($local:collection-path, "/manuscripts/")) then
                "https://jalit.org/manuscript/"
            else if (contains($local:collection-path, "/persons/")) then
                "https://jalit.org/person/"
            else if (contains($local:collection-path, "/works/")) then
                "https://jalit.org/work/"
            else if (contains($local:collection-path, "/places/")) then
                "https://jalit.org/place/"
            else if (contains($local:collection-path, "/relations/")) then
                "https://jalit.org/relation/"                
            else
                "https://jalit.org/"
    let $last-id := max(
        collection($local:collection-path)//tei:idno[@type eq 'URI'][parent::tei:publicationStmt]/local:extract-id-from-uri(.)
    )
        ...
        typeswitch($node)
            ...
            return
                element {node-name($node)} {
                    ...
                    local:transform($new, $id, $uri, $node/tei:editor),
...
declare function local:save($id as xs:string?, $file-name as xs:string, $post-processed-xml) as element(response) {
    try {
        let $save := xmldb:store($local:collection-path, xmldb:encode-uri($file-name), $post-processed-xml)
        return 
...
declare function local:git-commit($file-name as xs:string, $post-processed-xml) as element(response) {
    try {
        ...
        let $save-db := xmldb:store($local:collection-path, xmldb:encode-uri($file-name), $post-processed-xml)
        return
            <response status="okay" code="200">
