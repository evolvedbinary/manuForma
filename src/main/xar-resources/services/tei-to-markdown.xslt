...
<xsl:stylesheet
  ...
  <!-- Default: Identity trasform everything except Elements (which will be dealt with above) --> 
  <xsl:template match="node()[not(. instance of element())]|@*" mode="tei-to-markdown" priority="-3">
    <xsl:copy>
      <xsl:apply-templates select="node()" mode="#current"/>
    </xsl:copy>
  </xsl:template>
